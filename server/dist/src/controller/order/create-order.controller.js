...
const createOrder = async (req, res) => {
    try {
        ...
        const foodCalculations = await Promise.all(foods.map(async (item) => {
            const food = await models_1.FoodModel.findById(item.foodId);
            if (!food)
                throw new Error(`Хоол олдсонгүй: ${item.foodId}`);
            return {
                foodId: food._id,
        ...
        const newOrder = await models_1.OrderModel.create({
            user: userId,
